...
static void app_chooser_dialog_init(AppChooserDialog* dialog) {
    gtk_widget_init_template(GTK_WIDGET(dialog));

    gtk_list_box_set_header_func(GTK_LIST_BOX(dialog->list), update_header, NULL, NULL);
}

static void app_chooser_dialog_finalize(GObject* object) {
...
static void close_dialog(AppChooserDialog* dialog, GAppInfo* info) {
    dialog->info = info;
    g_signal_emit(dialog, signals[CLOSE], 0);
}

static void show_more(AppChooserDialog* dialog) {
    int i;

    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(dialog->scrolled_window), GTK_POLICY_NEVER,
...
static void row_activated(GtkListBox* list, GtkWidget* row, AppChooserDialog* dialog) {
    GAppInfo* info = NULL;
...
static void row_selected(GtkListBox* list, GtkWidget* row, AppChooserDialog* dialog) {
    gtk_widget_set_sensitive(dialog->open_button, TRUE);
    dialog->selected_row = row;
}

static void cancel_clicked(GtkWidget* button, AppChooserDialog* dialog) {
    close_dialog(dialog, NULL);
}

static void open_clicked(GtkWidget* button, AppChooserDialog* dialog) {
    GAppInfo* info = NULL;
...
static void show_error_dialog(gchar const* primary, gchar const* secondary, GtkWindow* parent) {
    GtkWidget* message_dialog;

    message_dialog = gtk_message_dialog_new(parent, 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, NULL);
    g_object_set(message_dialog, "text", primary, "secondary-text", secondary, NULL);
    gtk_dialog_set_default_response(GTK_DIALOG(message_dialog), GTK_RESPONSE_OK);
    gtk_widget_show(message_dialog);
    g_signal_connect(message_dialog, "response", G_CALLBACK(gtk_widget_destroy), NULL);
}

static void launch_software(AppChooserDialog* dialog) {
...
static void find_in_software(GtkWidget* button, AppChooserDialog* dialog) {
    launch_software(dialog);
}

static gboolean app_chooser_delete_event(GtkWidget* dialog, GdkEventAny* event) {
    close_dialog(APP_CHOOSER_DIALOG(dialog), NULL);

    return TRUE;
}

static void app_chooser_dialog_close(AppChooserDialog* dialog) {
    gtk_window_close(GTK_WINDOW(dialog));
}

static void app_chooser_dialog_class_init(AppChooserDialogClass* class) {
    ...
    signals[CLOSE] =
        g_signal_new("close", G_TYPE_FROM_CLASS(class), G_SIGNAL_ACTION | G_SIGNAL_RUN_LAST, 0,
    ...
    gtk_widget_class_set_template_from_resource(
        widget_class, "/org/freedesktop/portal/desktop/gtk/appchooserdialog.ui");
 ...
 */
static char* shorten_location(char const* location) {
    int len;
...
static void ensure_default_in_initial_list(char const** choices, char const* default_id) {
    int i;
    guint n_choices;
...
static void more_pressed(GtkGestureMultiPress* gesture, int n_press, double x, double y,
                         AppChooserDialog* dialog) {
    if (n_press != 1) {
        return;
    }

    if (gtk_list_box_get_row_at_y(GTK_LIST_BOX(dialog->list), y) ==
        GTK_LIST_BOX_ROW(dialog->more_row)) {
        show_more(dialog);
    }
}

AppChooserDialog* app_chooser_dialog_new(char const** choices, char const* default_id,
                                         char const* content_type, char const* location) {
    AppChooserDialog* dialog;
    int n_choices;
    int i;
    ...
    if (location) {
        ...
        heading =
            g_strdup_printf(_("Choose an application to open the file “%s”."), short_location);
    ...
    if (n_choices == 0) {
        ...
        if (location) {
            ...
            label = g_strdup_printf(_("No apps installed that can open “%s”. You can find more "
                                      "applications in Software"),
                                    short_location);
            gtk_label_set_label(GTK_LABEL(dialog->empty_label), label);
        } else {
            gtk_label_set_label(
                GTK_LABEL(dialog->empty_label),
                _("No suitable app installed. You can find more applications in Software."));
        }
    } else {
...
void app_chooser_dialog_update_choices(AppChooserDialog* dialog, char const** choices) {
    int i;
    ...
    for (i = 0; choices[i]; i++) {
        if (g_strv_contains((char const* const*) dialog->choices, choices[i])) {
            continue;
        }
