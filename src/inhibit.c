...
typedef enum { UNKNOWN = 0, RUNNING = 1, QUERY_END = 2, ENDING = 3 } SessionState;

static SessionState session_state = RUNNING;
static gboolean screensaver_active = FALSE;
static guint query_end_timeout;

static void uninhibit_done_gnome(GObject* source, GAsyncResult* result, gpointer data) {
...
static gboolean handle_close_gnome(XdpImplRequest* object, GDBusMethodInvocation* invocation,
                                   gpointer data) {
    ...
    if (cookie) {
        org_gnome_session_manager_call_uninhibit(sessionmanager, cookie, NULL, uninhibit_done_gnome,
                                                 NULL);
    } else {
...
static void inhibit_done_gnome(GObject* source, GAsyncResult* result, gpointer data) {
...
static gboolean handle_inhibit_gnome(XdpImplInhibit* object, GDBusMethodInvocation* invocation,
                                     gchar const* arg_handle, gchar const* arg_app_id,
                                     gchar const* arg_window, guint arg_flags,
    ...
    org_gnome_session_manager_call_inhibit(sessionmanager, arg_app_id, 0, /* window */
                                           reason, arg_flags, NULL, inhibit_done_gnome,
...
static void send_quit_response(GDBusProxy* client, gboolean will_quit, gchar const* reason) {
...
static void set_session_state(SessionState state) {
    char const* names[] = {"Unknown", "Running", "Query-end", "Ending"};
...
static void stop_waiting_for_query_end_response(gboolean send_response) {
...
static gboolean query_end_response(gpointer data) {
...
static void wait_for_query_end_response(GDBusProxy* proxy) {
    if (query_end_timeout != 0) {
...
static void maybe_send_quit_response(void) {
    if (query_end_timeout == 0) {
        return;
    }
...
static void client_proxy_signal(GDBusProxy* proxy, gchar const* sender_name,
                                gchar const* signal_name, GVariant* parameters,
                                gpointer user_data) {
...
static void uninhibit_done_fdo(GObject* source, GAsyncResult* result, gpointer data) {
    g_autoptr(GError) error = NULL;

    if (!org_freedesktop_screen_saver_call_un_inhibit_finish(fdo_screensaver, result, &error)) {
        g_warning("Backend call failed: %s", error->message);
    }
}

static gboolean handle_close_fdo(XdpImplRequest* object, GDBusMethodInvocation* invocation,
                                 gpointer data) {
    Request* request = (Request*) object;
    guint cookie;
    ...
    cookie = GPOINTER_TO_UINT(g_object_get_data(G_OBJECT(request), "cookie"));
    if (cookie) {
        org_freedesktop_screen_saver_call_un_inhibit(fdo_screensaver, cookie, NULL,
                                                     uninhibit_done_fdo, NULL);
    } else {
        g_object_set_data(G_OBJECT(request), "closed", GINT_TO_POINTER(1));
    }
...
static void inhibit_done_fdo(GObject* source, GAsyncResult* result, gpointer data) {
    g_autoptr(Request) request = data;
    guint cookie = 0;
    gboolean closed;
    g_autoptr(GError) error = NULL;

    if (!org_freedesktop_screen_saver_call_inhibit_finish(fdo_screensaver, &cookie, result,
                                                          &error)) {
        g_warning("Backend call failed: %s", error->message);
    }
...
static gboolean handle_inhibit_fdo(XdpImplInhibit* object, GDBusMethodInvocation* invocation,
                                   gchar const* arg_handle, gchar const* arg_app_id,
                                   gchar const* arg_window, guint arg_flags,
                                   GVariant* arg_options) {
    g_autoptr(Request) request = NULL;
    ...
    if ((arg_flags & ~INHIBIT_IDLE) != 0) {
        g_dbus_method_invocation_return_error(invocation, XDG_DESKTOP_PORTAL_ERROR,
    ...
    org_freedesktop_screen_saver_call_inhibit(fdo_screensaver, arg_app_id, reason, NULL,
                                              inhibit_done_fdo, g_object_ref(request));
...
static void emit_state_changed(Session* session) {
...
static void inhibit_session_close(Session* session) {
...
static void inhibit_session_finalize(GObject* object) {
    G_OBJECT_CLASS(inhibit_session_parent_class)->finalize(object);
}

static void inhibit_session_init(InhibitSession* inhibit_session) {}

static void inhibit_session_class_init(InhibitSessionClass* klass) {
...
static InhibitSession* inhibit_session_new(char const* app_id, char const* session_handle) {
    InhibitSession* inhibit_session;

    g_debug("Creating inhibit session %s", session_handle);

    inhibit_session = g_object_new(inhibit_session_get_type(), "id", session_handle, NULL);
...
static gboolean handle_create_monitor(XdpImplInhibit* object, GDBusMethodInvocation* invocation,
                                      char const* arg_handle, char const* arg_session_handle,
                                      char const* arg_app_id, char const* arg_window) {
    g_autoptr(GError) error = NULL;
    int response;
    Session* session;
...
static void global_active_changed_cb(GObject* object, gboolean active) {
...
gboolean inhibit_init(GDBusConnection* bus, GError** error) {
    ...
    sessionmanager = org_gnome_session_manager_proxy_new_sync(
        bus, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, "org.gnome.SessionManager",
        "/org/gnome/SessionManager", NULL, NULL);
    owner = g_dbus_proxy_get_name_owner(G_DBUS_PROXY(sessionmanager));

    if (owner) {
        screensaver = org_gnome_screen_saver_proxy_new_sync(bus, G_DBUS_PROXY_FLAGS_NONE,
                                                            "org.gnome.ScreenSaver",
                                                            "/org/gnome/ScreenSaver", NULL, NULL);
        ...
        if (owner2) {
            ...
            g_signal_connect(inhibit, "handle-create-monitor", G_CALLBACK(handle_create_monitor),
                             NULL);
            g_signal_connect(inhibit, "handle-query-end-response",
                             G_CALLBACK(handle_query_end_response), NULL);

            g_signal_connect(screensaver, "active-changed", G_CALLBACK(global_active_changed_cb),
                             NULL);
            ...
            if (org_gnome_session_manager_call_register_client_sync(
                    sessionmanager, "org.freedesktop.portal", "", &client_path, NULL, NULL)) {
                client =
                    g_dbus_proxy_new_sync(bus, 0, NULL, "org.gnome.SessionManager", client_path,
                                          "org.gnome.SessionManager.ClientPrivate", NULL, NULL);
    ...
    if (!screensaver) {
        ...
        fdo_screensaver = org_freedesktop_screen_saver_proxy_new_sync(
            bus, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, "org.freedesktop.ScreenSaver",
            "/org/freedesktop/ScreenSaver", NULL, NULL);

        g_signal_connect(fdo_screensaver, "active-changed", G_CALLBACK(global_active_changed_cb),
                         NULL);
