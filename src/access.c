...
static void access_dialog_handle_free(gpointer data) {
...
static void access_dialog_handle_close(AccessDialogHandle* handle) {
    gtk_widget_destroy(handle->dialog);
    access_dialog_handle_free(handle);
}

static void send_response(AccessDialogHandle* handle) {
    ...
    if (handle->response == 0 && handle->choices != NULL) {
        ...
        while (g_hash_table_iter_next(&iter, (gpointer*) &key, (gpointer*) &widget)) {
            if (GTK_IS_RADIO_BUTTON(widget)) {
                ...
                g_variant_builder_add(&choice_builder, "{sv}", str[0],
                                      g_variant_new_string(str[1]));
                g_strfreev(str);
            } else if (GTK_IS_CHECK_BUTTON(widget)) {
                gboolean active;

                active = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widget));
                g_variant_builder_add(&choice_builder, "{sv}", key,
                                      g_variant_new_string(active ? "true" : "false"));
            }
        }

        g_variant_builder_add(&opt_builder, "{sv}", "choices",
                              g_variant_builder_end(&choice_builder));
    }

    xdp_impl_access_complete_access_dialog(handle->impl, handle->invocation, handle->response,
                                           g_variant_builder_end(&opt_builder));

    access_dialog_handle_close(handle);
}

static void access_dialog_response(GtkWidget* widget, int response, gpointer user_data) {
...
static gboolean handle_close(XdpImplRequest* object, GDBusMethodInvocation* invocation,
    ...
    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
    xdp_impl_access_complete_access_dialog(handle->impl, handle->invocation, 2,
                                           g_variant_builder_end(&opt_builder));
    access_dialog_handle_close(handle);
    return FALSE;
}
...
static void add_choice(GtkWidget* box, GVariant* choice, GHashTable* table) {
    ...
    if (g_variant_n_children(options) == 0) {
        ...
        for (i = 0; i < g_variant_n_children(options); i++) {
            ...
            if (group) {
                gtk_radio_button_join_group(GTK_RADIO_BUTTON(radio), GTK_RADIO_BUTTON(group));
            } else {
                group = radio;
            }
...
static void fix_up_label_alignment(GtkWidget* area) {
    GList *children, *l;
...
static gboolean handle_access_dialog(XdpImplAccess* object, GDBusMethodInvocation* invocation,
                                     char const* arg_handle, char const* arg_app_id,
                                     char const* arg_parent_window, char const* arg_title,
                                     char const* arg_subtitle, char const* arg_body,
    ...
    gboolean modal;
    GtkWidget* dialog;
    char const* deny_label;
    char const* grant_label;
    char const* icon;
    ...
    dialog =
        gtk_message_dialog_new(NULL, 0, GTK_MESSAGE_QUESTION, GTK_BUTTONS_NONE, arg_title, NULL);
