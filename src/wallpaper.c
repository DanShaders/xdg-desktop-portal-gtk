...
static void wallpaper_dialog_handle_free(gpointer data) {
...
static void send_response(WallpaperDialogHandle* handle) {
    if (handle->request->exported) {
        request_unexport(handle->request);
    }

    xdp_impl_wallpaper_complete_set_wallpaper_uri(handle->impl, handle->invocation,
                                                  handle->response);

    wallpaper_dialog_handle_free(handle);
}

static gboolean set_gsettings(gchar* schema, gchar* uri) {
...
static void on_file_copy_cb(GObject* source_object, GAsyncResult* result, gpointer data) {
    WallpaperDialogHandle* handle = data;
    ...
    if (!g_file_replace_contents(destination, contents, length, NULL, FALSE,
...
static void set_wallpaper(WallpaperDialogHandle* handle, gchar const* uri) {
    ...
    g_file_load_contents_async(source, NULL, on_file_copy_cb, handle);
}

static void handle_wallpaper_dialog_response(WallpaperDialog* dialog, gint response,
                                             gpointer data) {
    WallpaperDialogHandle* handle = data;
...
static gboolean handle_set_wallpaper_uri(XdpImplWallpaper* object,
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(handle_wallpaper_dialog_response), handle);
