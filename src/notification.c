...
static void notification_added(GObject* source, GAsyncResult* result, gpointer data) {
...
static void handle_add_notification_gtk(XdpImplNotification* object,
                                        GDBusMethodInvocation* invocation, char const* arg_app_id,
                                        char const* arg_id, GVariant* arg_notification) {
    if (gtk_notifications) {
        org_gtk_notifications_call_add_notification(gtk_notifications, arg_app_id, arg_id,
                                                    arg_notification, NULL, notification_added,
                                                    NULL);
    }
...
static void handle_remove_notification_gtk(XdpImplNotification* object,
                                           GDBusMethodInvocation* invocation,
                                           char const* arg_app_id, char const* arg_id) {
    if (gtk_notifications) {
        org_gtk_notifications_call_remove_notification(gtk_notifications, arg_app_id, arg_id, NULL,
                                                       NULL, NULL);
    }
...
static char* app_path_for_id(gchar const* app_id) {
    char* path;
    gint i;
...
static void activate_action(GDBusConnection* connection, char const* app_id, char const* id,
    ...
    if (name && g_str_has_prefix(name, "app.")) {
        g_dbus_connection_call(
            connection, app_id, object_path, "org.freedesktop.Application", "ActivateAction",
            g_variant_new("(s@av@a{sv})", name + 4, g_variant_builder_end(&parms),
                          g_variant_builder_end(&pdata)),
            NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
    } else {
        g_autoptr(GVariant) ret = NULL;

        g_dbus_connection_call(connection, app_id, object_path, "org.freedesktop.Application",
                               "Activate", g_variant_new("(@a{sv})", g_variant_builder_end(&pdata)),
                               NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);

        g_dbus_connection_emit_signal(
            connection, NULL, "/org/freedesktop/portal/desktop",
            "org.freedesktop.impl.portal.Notification", "ActionInvoked",
            g_variant_new("(sss@av)", app_id, id, name, g_variant_builder_end(&parms)), NULL);
    }
}

static void handle_add_notification_fdo(XdpImplNotification* object,
                                        GDBusMethodInvocation* invocation, gchar const* arg_app_id,
                                        gchar const* arg_id, GVariant* arg_notification) {
...
static gboolean handle_remove_notification_fdo(XdpImplNotification* object,
                                               GDBusMethodInvocation* invocation,
                                               gchar const* arg_app_id, gchar const* arg_id) {
    GDBusConnection* connection;

    connection = g_dbus_method_invocation_get_connection(invocation);
    if (fdo_remove_notification(connection, arg_app_id, arg_id)) {
        g_debug("handle remove-notification from %s using the freedesktop implementation",
                arg_app_id);
        xdp_impl_notification_complete_remove_notification(object, invocation);
        return TRUE;
    }
...
static gboolean has_unprefixed_action(GVariant* notification) {
    char const* action;
...
static gboolean handle_add_notification(XdpImplNotification* object,
                                        GDBusMethodInvocation* invocation, gchar const* arg_app_id,
                                        gchar const* arg_id, GVariant* arg_notification) {
    g_autofree char* owner = gtk_notifications != NULL
                                 ? g_dbus_proxy_get_name_owner(G_DBUS_PROXY(gtk_notifications))
                                 : NULL;
    if (owner == NULL || !g_application_id_is_valid(arg_app_id) ||
...
static gboolean handle_remove_notification(XdpImplNotification* object,
                                           GDBusMethodInvocation* invocation,
                                           gchar const* arg_app_id, gchar const* arg_id) {
...
gboolean notification_init(GDBusConnection* bus, GError** error) {
    ...
    gtk_notifications = org_gtk_notifications_proxy_new_sync(
        bus, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, "org.gtk.Notifications",
        "/org/gtk/Notifications", NULL, NULL);
    ...
    g_signal_connect(helper, "handle-remove-notification", G_CALLBACK(handle_remove_notification),
                     NULL);
