...
static void install_dialog_handle_free(gpointer data) {
...
static void install_dialog_handle_close(InstallDialogHandle* handle) {
    if (handle->dialog) {
...
static gboolean handle_close(XdpImplRequest* object, GDBusMethodInvocation* invocation,
    ...
    xdp_impl_dynamic_launcher_complete_prepare_install(handle->impl, handle->invocation, 2,
...
static void send_prepare_install_response(InstallDialogHandle* handle) {
    GVariantBuilder opt_builder;

    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);

    if (handle->response == 0) {
        char const* chosen_name = gtk_entry_get_text(GTK_ENTRY(handle->entry));
    ...
    xdp_impl_dynamic_launcher_complete_prepare_install(
        handle->impl, handle->invocation, handle->response, g_variant_builder_end(&opt_builder));

    install_dialog_handle_close(handle);
}

static void handle_prepare_install_response(GtkDialog* dialog, gint response, gpointer data) {
    InstallDialogHandle* handle = data;
...
static gboolean handle_prepare_install(XdpImplDynamicLauncher* object,
                                       GDBusMethodInvocation* invocation, char const* arg_handle,
                                       char const* arg_app_id, char const* arg_parent_window,
                                       char const* arg_name, GVariant* arg_icon_v,
    ...
    dialog = gtk_dialog_new_with_buttons(title, GTK_WINDOW(fake_parent), dialog_flags, _("_Cancel"),
                                         GTK_RESPONSE_CANCEL, _("C_reate"), GTK_RESPONSE_OK, NULL);
    ...
    if (icon_stream) {
        pixbuf = gdk_pixbuf_new_from_stream_at_scale(icon_stream, DEFAULT_ICON_SIZE,
                                                     DEFAULT_ICON_SIZE, TRUE, NULL, &error);
    }
    if (error) {
...
static gboolean handle_request_install_token(XdpImplDynamicLauncher* object,
                                             GDBusMethodInvocation* invocation,
                                             char const* arg_app_id, GVariant* arg_options) {
    /* Generally these should be allowed in each desktop specific x-d-p backend,
     * but add them here as well as a fallback.
     */
    static char const* allowlist[] = {"org.gnome.Software", "org.gnome.SoftwareDevel",
                                      "io.elementary.appcenter", "org.kde.discover", NULL};
    guint response;
    ...
    xdp_impl_dynamic_launcher_complete_request_install_token(object, invocation, response);
    return TRUE;
}

gboolean dynamic_launcher_init(GDBusConnection* bus, GError** error) {
    GDBusInterfaceSkeleton* helper;
    DynamicLauncherType supported_types =
        DYNAMIC_LAUNCHER_TYPE_APPLICATION | DYNAMIC_LAUNCHER_TYPE_WEBAPP;
    ...
    g_signal_connect(helper, "handle-request-install-token",
                     G_CALLBACK(handle_request_install_token), NULL);
