...
static void wallpaper_dialog_apply(WallpaperDialog* self) {
    g_signal_emit(self, signals[RESPONSE], 0, GTK_RESPONSE_APPLY);
}

static void wallpaper_dialog_cancel(WallpaperDialog* self) {
    g_signal_emit(self, signals[RESPONSE], 0, GTK_RESPONSE_CANCEL);
}

static void wallpaper_dialog_finalize(GObject* object) {
...
static void wallpaper_dialog_init(WallpaperDialog* self) {
...
static void wallpaper_dialog_class_init(WallpaperDialogClass* klass) {
    ...
    signals[RESPONSE] =
        g_signal_new("response", G_TYPE_FROM_CLASS(klass), G_SIGNAL_ACTION | G_SIGNAL_RUN_LAST, 0,
                     NULL, NULL, NULL, G_TYPE_NONE, 1, G_TYPE_INT);

    gtk_widget_class_set_template_from_resource(
        widget_class, "/org/freedesktop/portal/desktop/gtk/wallpaperdialog.ui");
...
static void on_image_loaded_cb(GObject* source_object, GAsyncResult* result, gpointer data) {
    ...
    wallpaper_preview_set_image(self->desktop_preview, self->picture_uri);
}

WallpaperDialog* wallpaper_dialog_new(gchar const* picture_uri, gchar const* app_id) {
    ...
    g_file_load_contents_async(image_file, NULL, on_image_loaded_cb, self);

    return self;
}

gchar const* wallpaper_dialog_get_uri(WallpaperDialog* dialog) {
