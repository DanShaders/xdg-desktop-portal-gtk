...
static SettingsBundle* settings_bundle_new(GSettingsSchema* schema, GSettings* settings) {
...
static void settings_bundle_free(SettingsBundle* bundle) {
    g_object_unref(bundle->schema);
    g_object_unref(bundle->settings);
    g_free(bundle);
}

static gboolean namespace_matches(char const* namespace, char const* const* patterns) {
    size_t i;
...
static gboolean settings_handle_read_all(XdpImplSettings* object, GDBusMethodInvocation* invocation,
                                         char const* const* arg_namespaces, gpointer data) {
    ...
    while (g_hash_table_iter_next(&iter, (gpointer*) &key, (gpointer*) &value)) {
        ...
        for (i = 0; keys[i]; ++i) {
            if (strcmp(key, "org.gnome.desktop.interface") == 0 &&
                ...
                g_variant_dict_insert_value(&dict, keys[i],
                                            g_variant_new_boolean(enable_animations));
            } else {
                g_variant_dict_insert_value(&dict, keys[i],
                                            g_settings_get_value(value->settings, keys[i]));
            }
        }
    ...
    if (namespace_matches("org.gnome.fontconfig", arg_namespaces)) {
        ...
        g_variant_builder_add(builder, "{s@a{sv}}", "org.gnome.fontconfig",
...
static gboolean settings_handle_read(XdpImplSettings* object, GDBusMethodInvocation* invocation,
                                     char const* arg_namespace, char const* arg_key,
                                     gpointer data) {
    ...
    if (strcmp(arg_namespace, "org.gnome.fontconfig") == 0) {
        if (strcmp(arg_key, "serial") == 0) {
            g_dbus_method_invocation_return_value(
                invocation, g_variant_new("(v)", g_variant_new_int32(fontconfig_serial)));
            return TRUE;
        }
    } else if (strcmp(arg_namespace, "org.freedesktop.appearance") == 0 &&
               strcmp(arg_key, "color-scheme") == 0) {
        g_dbus_method_invocation_return_value(invocation, g_variant_new("(v)", get_color_scheme()));
        return TRUE;
    } else if (strcmp(arg_namespace, "org.gnome.desktop.interface") == 0 &&
               strcmp(arg_key, "enable-animations") == 0) {
        g_dbus_method_invocation_return_value(
            invocation, g_variant_new("(v)", g_variant_new_boolean(enable_animations)));
        return TRUE;
...
static ChangedSignalUserData* changed_signal_user_data_new(XdpImplSettings* settings,
                                                           char const* namespace) {
...
static void changed_signal_user_data_destroy(gpointer data, GClosure* closure) {
    g_free(data);
}

static void on_settings_changed(GSettings* settings, char const* key,
    ...
    if (strcmp(user_data->namespace, "org.gnome.desktop.interface") == 0 &&
        ...
        xdp_impl_settings_emit_setting_changed(user_data->self, user_data->namespace, key,
                                               g_variant_new("v", new_value));
    }

    if (strcmp(user_data->namespace, "org.gnome.desktop.interface") == 0 &&
        strcmp(key, "color-scheme") == 0) {
        xdp_impl_settings_emit_setting_changed(user_data->self, "org.freedesktop.appearance", key,
                                               g_variant_new("v", get_color_scheme()));
    }
}

static void init_settings_table(XdpImplSettings* settings, GHashTable* table) {
    ...
    for (i = 0; i < G_N_ELEMENTS(schemas); ++i) {
        ...
        char const* schema_name = schemas[i];
...
static void fontconfig_changed(FcMonitor* monitor, XdpImplSettings* impl) {
    char const* namespace = "org.gnome.fontconfig";
    char const* key = "serial";
    ...
    xdp_impl_settings_emit_setting_changed(
        impl, namespace, key, g_variant_new("v", g_variant_new_int32(fontconfig_serial)));
}

static void set_enable_animations(XdpImplSettings* impl, gboolean new_enable_animations) {
    char const* namespace = "org.gnome.desktop.interface";
    char const* key = "enable-animations";
    ...
    enable_animations_variant = g_variant_new("v", g_variant_new_boolean(enable_animations));
    xdp_impl_settings_emit_setting_changed(impl, namespace, key, enable_animations_variant);
}
...
gboolean settings_init(GDBusConnection* bus, GError** error) {
    ...
    settings =
        g_hash_table_new_full(g_str_hash, g_str_equal, NULL, (GDestroyNotify) settings_bundle_free);
