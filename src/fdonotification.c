...
static void fdo_notification_free(gpointer data) {
...
FdoNotification* fdo_find_notification(char const* app_id, char const* id) {
    GSList* l;

    for (l = fdo_notifications; l != NULL; l = l->next) {
        FdoNotification* n = l->data;
        if (g_str_equal(n->app_id, app_id) && g_str_equal(n->id, id)) {
            return n;
        }
    }
...
FdoNotification* fdo_find_notification_by_notify_id(guint32 id) {
    GSList* l;
...
static void notify_signal(GDBusConnection* connection, char const* sender_name,
                          char const* object_path, char const* interface_name,
                          char const* signal_name, GVariant* parameters, gpointer user_data) {
    guint32 id = 0;
    ...
    if (action) {
        if (g_str_equal(action, "default")) {
            n->activate_action(connection, n->app_id, n->id, n->default_action,
                               n->default_action_target, n->data);
        } else {
            gchar* name;
            GVariant* target;

            if (g_action_parse_detailed_name(action, &name, &target, NULL)) {
                n->activate_action(connection, n->app_id, n->id, name, target, n->data);
                g_free(name);
                if (target) {
...
static guchar urgency_from_priority(char const* priority) {
...
static void call_notify(GDBusConnection* connection, FdoNotification* fdo, GVariant* notification) {
    ...
    if (fdo_notify_subscription == 0) {
        fdo_notify_subscription = g_dbus_connection_signal_subscribe(
            connection, "org.freedesktop.Notifications", "org.freedesktop.Notifications", NULL,
            "/org/freedesktop/Notifications", NULL, G_DBUS_SIGNAL_FLAGS_NONE, notify_signal, NULL,
            NULL);
    }
    ...
    if (icon != NULL) {
        ...
        if (G_IS_FILE_ICON(gicon)) {
            ...
            gchar const* const* icon_names = g_themed_icon_get_names(G_THEMED_ICON(gicon));
            ...
            istream = g_loadable_icon_load(G_LOADABLE_ICON(gicon), -1 /* unused */, NULL /* type */,
                                           NULL, NULL);
            pixbuf = gdk_pixbuf_new_from_stream(istream, NULL, NULL);
            g_input_stream_close(istream, NULL, NULL);

            g_object_get(pixbuf, "width", &width, "height", &height, "rowstride", &rowstride,
                         "n-channels", &n_channels, "bits-per-sample", &bits_per_sample, NULL);

            image_len = (height - 1) * rowstride + width * ((n_channels * bits_per_sample + 7) / 8);

            image =
                g_variant_new("(iiibii@ay)", width, height, rowstride,
                              gdk_pixbuf_get_has_alpha(pixbuf), bits_per_sample, n_channels,
                              g_variant_new_from_data(
                                  G_VARIANT_TYPE("ay"), gdk_pixbuf_get_pixels(pixbuf), image_len,
    ...
    g_dbus_connection_call(
        ...
        g_variant_new("(susssasa{sv}i)", "", /* app name */
                      fdo->notify_id, icon_name, title, body, &action_builder, &hints_builder,
                      -1), /* expire_timeout */
        G_VARIANT_TYPE("(u)"), G_DBUS_CALL_FLAGS_NONE, -1, NULL, notification_sent, fdo);
}

static void call_close(GDBusConnection* connection, guint32 id) {
    g_dbus_connection_call(connection, "org.freedesktop.Notifications",
...
void fdo_add_notification(GDBusConnection* connection, char const* app_id, char const* id,
                          GVariant* notification, ActivateAction activate_action, gpointer data) {
    FdoNotification* n;
