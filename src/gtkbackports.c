...
static void add_to_variant(gchar const* key, gchar const* value, gpointer data) {
...
GVariant* gtk_paper_size_to_gvariant(GtkPaperSize* paper_size) {
    ...
    g_variant_builder_add(&builder, "{sv}", "Width",
                          g_variant_new_double(gtk_paper_size_get_width(paper_size, GTK_UNIT_MM)));
    g_variant_builder_add(&builder, "{sv}", "Height",
                          g_variant_new_double(gtk_paper_size_get_height(paper_size, GTK_UNIT_MM)));
...
GtkPaperSize* gtk_paper_size_new_from_gvariant(GVariant* variant) {
    ...
    if (ppd_name != NULL) {
        paper_size = gtk_paper_size_new_from_ppd(
            ppd_name, display_name, _gtk_print_convert_from_mm(width, GTK_UNIT_POINTS),
            _gtk_print_convert_from_mm(height, GTK_UNIT_POINTS));
    } else if (name != NULL) {
        paper_size = gtk_paper_size_new_custom(name, display_name, width, height, GTK_UNIT_MM);
    } else {
        paper_size = NULL;
    }
...
static char* enum_to_string(GType type, guint enum_value) {
...
static guint string_to_enum(GType type, char const* enum_string) {
    GEnumClass* enum_class;
    GEnumValue const* value;
    guint retval = 0;
...
GVariant* gtk_page_setup_to_gvariant(GtkPageSetup* setup) {
    ...
    g_variant_builder_add(&builder, "{sv}", "MarginTop",
                          g_variant_new_double(gtk_page_setup_get_top_margin(setup, GTK_UNIT_MM)));
    g_variant_builder_add(
        &builder, "{sv}", "MarginBottom",
        g_variant_new_double(gtk_page_setup_get_bottom_margin(setup, GTK_UNIT_MM)));
    g_variant_builder_add(&builder, "{sv}", "MarginLeft",
                          g_variant_new_double(gtk_page_setup_get_left_margin(setup, GTK_UNIT_MM)));
    g_variant_builder_add(
        &builder, "{sv}", "MarginRight",
        g_variant_new_double(gtk_page_setup_get_right_margin(setup, GTK_UNIT_MM)));

    orientation = enum_to_string(GTK_TYPE_PAGE_ORIENTATION, gtk_page_setup_get_orientation(setup));
...
GtkPageSetup* gtk_page_setup_new_from_gvariant(GVariant* variant) {
    ...
    if (g_variant_lookup(variant, "Orientation", "&s", &orientation)) {
        gtk_page_setup_set_orientation(setup,
                                       string_to_enum(GTK_TYPE_PAGE_ORIENTATION, orientation));
    }
