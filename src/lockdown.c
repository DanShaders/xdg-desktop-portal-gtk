...
gboolean lockdown_init(GDBusConnection* bus, GError** error) {
    ...
    g_settings_bind(lockdown, "disable-printing", helper, "disable-printing",
                    G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(lockdown, "disable-save-to-disk", helper, "disable-save-to-disk",
                    G_SETTINGS_BIND_DEFAULT);
    g_settings_bind(lockdown, "disable-application-handlers", helper,
                    "disable-application-handlers", G_SETTINGS_BIND_DEFAULT);

    location = g_settings_new("org.gnome.system.location");
    g_settings_bind(location, "enabled", helper, "disable-location",
    ...
    if (g_settings_schema_has_key(schema, "disable-camera")) {
        g_settings_bind(privacy, "disable-camera", helper, "disable-camera",
                        G_SETTINGS_BIND_DEFAULT);
    }
    if (g_settings_schema_has_key(schema, "disable-microphone")) {
        g_settings_bind(privacy, "disable-microphone", helper, "disable-microphone",
                        G_SETTINGS_BIND_DEFAULT);
    }
