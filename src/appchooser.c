...
static void app_dialog_handle_free(gpointer data) {
...
static void app_dialog_handle_close(AppDialogHandle* handle) {
    gtk_widget_destroy(handle->dialog);
    app_dialog_handle_free(handle);
}

static void send_response(AppDialogHandle* handle) {
    GVariantBuilder opt_builder;

    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
    g_variant_builder_add(&opt_builder, "{sv}", "choice",
                          g_variant_new_string(handle->chosen ? handle->chosen : ""));
...
static void handle_app_chooser_close(AppChooserDialog* dialog, gpointer data) {
    AppDialogHandle* handle = data;
    GAppInfo* info;

    info = app_chooser_dialog_get_info(dialog);
    if (info != NULL) {
        char const* desktop_id = g_app_info_get_id(info);
        handle->response = 0;
...
static gboolean handle_close(XdpImplRequest* object, GDBusMethodInvocation* invocation,
    ...
    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
    xdp_impl_app_chooser_complete_choose_application(handle->impl, handle->invocation, 2,
...
static gboolean handle_choose_application(XdpImplAppChooser* object,
                                          GDBusMethodInvocation* invocation, char const* arg_handle,
                                          char const* arg_app_id, char const* arg_parent_window,
                                          char const** choices, GVariant* arg_options) {
    g_autoptr(Request) request = NULL;
    GtkWidget* dialog;
    AppDialogHandle* handle;
    char const* sender;
    char const* latest_chosen_id;
    ...
    g_signal_connect(dialog, "close", G_CALLBACK(handle_app_chooser_close), handle);
...
static gboolean handle_update_choices(XdpImplAppChooser* object, GDBusMethodInvocation* invocation,
                                      char const* arg_handle, char const** choices) {
    AppDialogHandle* handle;
...
gboolean app_chooser_init(GDBusConnection* bus, GError** error) {
    ...
    g_signal_connect(helper, "handle-choose-application", G_CALLBACK(handle_choose_application),
                     NULL);
