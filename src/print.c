...
static void print_params_free(gpointer data) {
...
static void ensure_print_params(void) {
    if (print_params) {
        return;
    }
...
static gboolean print_params_timeout(gpointer data) {
    print_params_free(data);
    g_debug("Removing print params, now %d", g_hash_table_size(print_params));
    return G_SOURCE_REMOVE;
}

static PrintParams* print_params_new(char const* app_id, gboolean preview, GtkPageSetup* page_setup,
                                     GtkPrintSettings* settings, GtkPrinter* printer) {
    PrintParams* params;
    guint32 r;
    ...
    g_debug("Remembering print params for %s, token %u, now %d", params->app_id, params->token,
...
static PrintParams* get_print_params(char const* app_id, guint32 token) {
    PrintParams* params;
...
static void print_dialog_handle_free(gpointer data) {
...
static void print_dialog_handle_close(PrintDialogHandle* handle) {
    gtk_widget_destroy(handle->dialog);
    print_dialog_handle_free(handle);
}

static gboolean can_preview(void) {
...
static gboolean launch_preview(char const* filename, char const* title, GtkPrintSettings* settings,
                               GtkPageSetup* page_setup, GError** error) {
    ...
    appinfo =
        g_app_info_create_from_commandline(cmd, "Print Preview", G_APP_INFO_CREATE_NONE, error);
    if (!appinfo) {
        goto out;
    }
...
static gboolean print_file(int fd, char const* app_id, gboolean preview, GtkPrinter* printer,
                           GtkPrintSettings* settings, GtkPageSetup* page_setup, GError** error) {
    ...
    if (g_output_stream_splice(G_OUTPUT_STREAM(ostream), G_INPUT_STREAM(istream),
...
static void handle_print_response(GtkDialog* dialog, gint response, gpointer data) {
    PrintDialogHandle* handle = data;
    ...
    switch (response) {
        ...
        case GTK_RESPONSE_OK: {
            ...
            if (!print_file(handle->fd, handle->request->app_id, preview, printer, settings,
                            page_setup, &error)) {
                handle->response = 2;
            } else {
                handle->response = 0;
    ...
    if (error) {
        ...
        xdp_impl_print_complete_print(handle->impl, handle->invocation, NULL, handle->response,
...
static gboolean handle_close(XdpImplRequest* object, GDBusMethodInvocation* invocation,
    ...
    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
    xdp_impl_print_complete_print(handle->impl, handle->invocation, NULL, 2,
                                  g_variant_builder_end(&opt_builder));
...
static gboolean handle_print(XdpImplPrint* object, GDBusMethodInvocation* invocation,
                             GUnixFDList* fd_list, char const* arg_handle, char const* arg_app_id,
                             char const* arg_parent_window, char const* arg_title,
                             GVariant* arg_fd_in, GVariant* arg_options) {
    ...
    if (params) {
        ...
        print_file(fd, params->app_id, params->preview, params->printer, params->settings,
                   params->page_setup, NULL);

        print_params_free(params);

        g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
        xdp_impl_print_complete_print(object, invocation, NULL, 0,
                                      g_variant_builder_end(&opt_builder));
        return TRUE;
    }
...
static void send_prepare_print_response(PrintDialogHandle* handle) {
    GVariantBuilder opt_builder;

    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);

    if (handle->response == 0) {
        g_variant_builder_add(&opt_builder, "{sv}", "settings",
                              gtk_print_settings_to_gvariant(handle->params->settings));
        g_variant_builder_add(&opt_builder, "{sv}", "page-setup",
                              gtk_page_setup_to_gvariant(handle->params->page_setup));
        g_variant_builder_add(&opt_builder, "{sv}", "token",
                              g_variant_new_uint32(handle->params->token));
    }
    ...
    xdp_impl_print_complete_prepare_print(handle->impl, handle->invocation, handle->response,
                                          g_variant_builder_end(&opt_builder));

    print_dialog_handle_close(handle);
}

static void handle_prepare_print_response(GtkDialog* dialog, gint response, gpointer data) {
    PrintDialogHandle* handle = data;
    gboolean preview = FALSE;
    ...
    switch (response) {
        ...
        case GTK_RESPONSE_OK: {
            ...
            handle->params = print_params_new(
                handle->request->app_id, preview,
                gtk_print_unix_dialog_get_page_setup(GTK_PRINT_UNIX_DIALOG(handle->dialog)),
...
static gboolean handle_prepare_print(XdpImplPrint* object, GDBusMethodInvocation* invocation,
                                     char const* arg_handle, char const* arg_app_id,
                                     char const* arg_parent_window, char const* arg_title,
                                     GVariant* arg_settings, GVariant* arg_page_setup,
