...
static void fontconfig_cache_update_thread(GTask* task, gpointer source_object G_GNUC_UNUSED,
    ...
    if (!FcInitReinitialize()) {
        g_task_return_new_error(task, G_IO_ERROR, G_IO_ERROR_FAILED, "FcInitReinitialize failed");
        return;
    }
...
static void fontconfig_cache_update_async(GAsyncReadyCallback callback, gpointer user_data) {
    GTask* task = g_task_new(NULL, NULL, callback, user_data);
    g_task_run_in_thread(task, fontconfig_cache_update_thread);
    g_object_unref(task);
}

static gboolean fontconfig_cache_update_finish(GAsyncResult* result, GError** error) {
...
static void fc_monitor_class_init(FcMonitorClass* klass) {
    GObjectClass* object_class = G_OBJECT_CLASS(klass);

    object_class->finalize = fc_monitor_finalize;

    signals[SIGNAL_UPDATED] = g_signal_new("updated", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
...
static void fc_monitor_init(FcMonitor* self G_GNUC_UNUSED) {
    FcInit();
}
...
void fc_monitor_start(FcMonitor* self) {
...
void fc_monitor_stop(FcMonitor* self) {
    g_return_if_fail(FC_IS_MONITOR(self));
    g_clear_pointer(&self->monitors, g_ptr_array_unref);
}
...
static void monitor_files(FcMonitor* self, FcStrList* list) {
    ...
    while ((str = (char const*) FcStrListNext(list))) {
        GFile* file;
...
static gchar const* get_name(GType enum_type, gint enum_value) {
    GEnumClass* klass = g_type_class_ref(enum_type);
    GEnumValue* value = g_enum_get_value(klass, enum_value);
    gchar const* name = value ? value->value_name : "(unknown)";
    g_type_class_unref(klass);
    return name;
}

static void stuff_changed(GFileMonitor* monitor G_GNUC_UNUSED, GFile* file G_GNUC_UNUSED,
                          GFile* other_file G_GNUC_UNUSED, GFileMonitorEvent event_type,
                          gpointer data) {
    FcMonitor* self = FC_MONITOR(data);
    gchar const* event_name = get_name(G_TYPE_FILE_MONITOR_EVENT, event_type);
...
static void update_done(GObject* source_object G_GNUC_UNUSED, GAsyncResult* result, gpointer data) {
    FcMonitor* self = FC_MONITOR(data);
