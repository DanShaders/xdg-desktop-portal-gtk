...
static void add_recent_entry(char const* app_id, char const* uri) {
...
static void send_response(FileDialogHandle* handle) {
    ...
    if (strcmp(method_name, "SaveFiles") == 0 &&
        handle->action == GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER && handle->uris) {
    ...
    if (strcmp(method_name, "OpenFile") == 0) {
        xdp_impl_file_chooser_complete_open_file(handle->impl, handle->invocation, handle->response,
                                                 g_variant_builder_end(&opt_builder));
    } else if (strcmp(method_name, "SaveFile") == 0) {
        xdp_impl_file_chooser_complete_save_file(handle->impl, handle->invocation, handle->response,
                                                 g_variant_builder_end(&opt_builder));
    } else if (strcmp(method_name, "SaveFiles") == 0) {
        xdp_impl_file_chooser_complete_save_files(handle->impl, handle->invocation,
                                                  handle->response,
                                                  g_variant_builder_end(&opt_builder));
    } else {
...
static gboolean handle_close(XdpImplRequest* object, GDBusMethodInvocation* invocation,
    ...
    if (handle->action == GTK_FILE_CHOOSER_ACTION_OPEN) {
        xdp_impl_file_chooser_complete_open_file(handle->impl, handle->invocation, 2,
                                                 g_variant_builder_end(&opt_builder));
    } else if (handle->action == GTK_FILE_CHOOSER_ACTION_SAVE) {
        xdp_impl_file_chooser_complete_save_file(handle->impl, handle->invocation, 2,
                                                 g_variant_builder_end(&opt_builder));
    } else {
        xdp_impl_file_chooser_complete_save_files(handle->impl, handle->invocation, 2,
                                                  g_variant_builder_end(&opt_builder));
    }
    ...
    if (g_variant_lookup(arg_options, "current_filter", "@(sa(us))", &current_filter)) {
        ...
        if (!filters) {
            ...
            for (GSList* l = filters; l; l = l->next) {
                ...
                char const* name = gtk_file_filter_get_name(f);
            ...
            if (!handled) {
                g_warning(
                    "current file filter must be present in filters list when list is nonempty");
            }
        }
    }
