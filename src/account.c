...
static void account_dialog_handle_free(gpointer data) {
...
static void account_dialog_handle_close(AccountDialogHandle* handle) {
    gtk_widget_destroy(handle->dialog);
    account_dialog_handle_free(handle);
}

static void send_response(AccountDialogHandle* handle) {
    GVariantBuilder opt_builder;

    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
    g_variant_builder_add(&opt_builder, "{sv}", "id",
                          g_variant_new_string(handle->user_name ? handle->user_name : ""));
    g_variant_builder_add(&opt_builder, "{sv}", "name",
                          g_variant_new_string(handle->real_name ? handle->real_name : ""));
    g_variant_builder_add(&opt_builder, "{sv}", "image",
                          g_variant_new_string(handle->icon_uri ? handle->icon_uri : ""));
...
static void account_dialog_done(GtkWidget* widget, int response, char const* user_name,
                                char const* real_name, char const* icon_file, gpointer user_data) {
...
static gboolean handle_close(XdpImplRequest* object, GDBusMethodInvocation* invocation,
    ...
    g_variant_builder_init(&opt_builder, G_VARIANT_TYPE_VARDICT);
    xdp_impl_account_complete_get_user_information(handle->impl, handle->invocation, 2,
...
static gboolean handle_get_user_information(XdpImplAccount* object,
                                            GDBusMethodInvocation* invocation,
...
gboolean account_init(GDBusConnection* bus, GError** error) {
    ...
    g_signal_connect(helper, "handle-get-user-information", G_CALLBACK(handle_get_user_information),
                     NULL);
    ...
    user = org_freedesktop_accounts_user_proxy_new_sync(
        system_bus, G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START, "org.freedesktop.Accounts", object_path,
        NULL, error);
    if (user == NULL) {
        return FALSE;
    }
