...
static void account_dialog_init(AccountDialog* dialog) {
...
static gboolean account_dialog_delete_event(GtkWidget* dialog, GdkEventAny* event) {
...
static void button_clicked(GtkWidget* button, AccountDialog* dialog) {
    int response;
...
static void dialog_set_icon_file(AccountDialog* dialog, char const* icon_file) {
    g_clear_pointer(&dialog->icon_file, g_free);
...
static void image_button_clicked(AccountDialog* dialog) {
    GtkWidget* chooser;
    GtkWidget* preview;
    GtkFileFilter* filter;

    chooser = gtk_file_chooser_dialog_new(
        _("Select an Image"), GTK_WINDOW(dialog), GTK_FILE_CHOOSER_ACTION_OPEN, _("Cancel"),
        GTK_RESPONSE_CANCEL, _("Select"), GTK_RESPONSE_OK, _("Clear"), GTK_RESPONSE_CLOSE, NULL);
...
static void override_button_clicked(GtkButton* button, AccountDialog* dialog) {
...
static void account_dialog_class_init(AccountDialogClass* class) {
    ...
    signals[DONE] =
        g_signal_new("done", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL,
                     G_TYPE_NONE, 4, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

    gtk_widget_class_set_template_from_resource(
        widget_class, "/org/freedesktop/portal/desktop/gtk/accountdialog.ui");
...
AccountDialog* account_dialog_new(char const* app_id, char const* user_name, char const* real_name,
    ...
    if (strcmp(app_id, "") != 0) {
        ...
        heading = g_strdup_printf(_("Share your personal information with %s?"),
